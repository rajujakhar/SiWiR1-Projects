...
int main(int argc, char *argv[])
{
  ...
  while(count < num_iter)
  {
    ...
    if(cart_rank ==0)
    {
      ...
      MPI_Isend((d+sizeproc-numgridpoints_x),numgridpoints_x,MPI_DOUBLE,up,2,new_comm,&request[0]);
      std::cout<<"I am "<<std::endl;
    }

    else if(up>0)
    {
      MPI_Isend(d+sizeproc,numgridpoints_x,MPI_DOUBLE,up,2,new_comm,&request[1]);
    }
    ...
    if(down>=0)
    {
      MPI_Isend(d+numgridpoints_x,numgridpoints_x,MPI_DOUBLE,down,3,new_comm,&request[2]);
    }
