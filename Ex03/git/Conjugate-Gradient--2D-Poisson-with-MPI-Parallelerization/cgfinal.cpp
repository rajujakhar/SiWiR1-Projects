...
int main(int argc, char *argv[])
{
    ...
    if(cart_rank == 0)
    {
      std::cout<<"(1)I am Rank "<<rank<<" and I have received "<<sizeproc<<"memeory for computation"<<std::endl;
      if (domain[2]==0)
      {
    ...
    //std::cout<<" parameters done upto line number 125"<<std::endl;

    //double d[2*numgridpoints_x + sizeproc]={0.0};
    std::vector<double> d(2*numgridpoints_x + sizeproc,0.0);

    //std::cout<<" parameters done upto line number 129"<<std::endl;

    //double z[2*numgridpoints_x + sizeproc]={0.0};
    std::vector<double> z(2*numgridpoints_x + sizeproc,0.0);

    //std::cout<<" parameters done upto line number 128"<<std::endl;
    ...
    if(cart_rank==0)
    {
      //std::cout<<" Done till here  200 "<<std::endl;

      //std::vector<double> r_0(numgridpoints_x*numgridpoints_y,0.0);

      //std::cout<<" Done till here  203 "<<std::endl;
  ...
  else
  {
    ...
    MPI_Recv(&r[0],sizeproc,MPI_DOUBLE,0,0,new_comm,&status1);
    MPI_Recv(&u[0],sizeproc,MPI_DOUBLE,0,1,new_comm,&status2);
  }  

  std::cout<<" Done till line 278"<<std::endl;
  ...
  {
    //std::cout<<"I am rank  "<<cart_rank<<std::endl;
    for (auto i = 0; i < sizeproc; ++i) error: ‘status’ was not declared in this scope
                                               MPI_Recv(d.data()+ sizeproc,numgridpoints_x,MPI_DOUBLE,up,3,new_comm,&status[1]);
  ...
  std::cout<<" ASSIGNMENT of d done"<<std::endl;
  //************************************************
  ...
  std::cout<<"Compute inner Indexes done"<<std::endl;
  ...
  while(count < num_iter)
  {
    ...
    std::cout<<"status declared"<<std::endl;
    ...
    if(cart_rank ==0)
    {
      std::cout<<"sendding up from 0"<<std::endl;
    ...
    std::cout<<"Send up done"<<std::endl;
    ...
    if(down>=0 )
    {
      MPI_Recv(d.data(),numgridpoints_x,MPI_DOUBLE,down,2,new_comm,&status[0]);
    ...
    {
      MPI_Recv( d.data()+ sizeproc + numgridpoints_x,numgridpoints_x,MPI_DOUBLE,up,3,new_comm,&status[2]);
