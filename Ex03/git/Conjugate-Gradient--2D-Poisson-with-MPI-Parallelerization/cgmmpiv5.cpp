...
int main(int argc, char *argv[])
{
  ...
  MPI_Comm_rank( MPI_COMM_WORLD, &rank );
    ...
    double u_final[numgridpoints_x*numgridpoints_y]={0.0};
    int new_x=0;
    int new_y=0;
    double delta_0=0.;
  ...
  while(count < num_iter)
  {
   ...
   // std::cout << "Iteration " << count << "has just started\n";
    ...
    // if(cart_rank==2){
    //  for (int i = 0; i < sizeproc+ 1*numgridpoints_x; ++i)
    //    {
    //      std::cout<<d[i]<<std::endl;
    //    }
    // }
   ...
   // std::cout << "My rank is " << cart_rank  << " and my (d,z) is: " << scalar_Product_d_z << std::endl; 
    ...
    if(cart_rank==0)
    {
     std::cout << "\nResidual for iter: " << count+1 << "is: " << delta_1 << "\n" ;
   ...
   if(cart_rank == 0) delta_0 = delta_1; 
   count++;
   //if(isSolutionConverged == 1)
   // count = num_iter-1;
