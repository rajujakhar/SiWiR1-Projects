...
int main(int argc, char *argv[])
{
  ...
  // {
   ...
   // }    
    ...
    //std::cout << "delta_0_local for " << cart_rank << " : " << delta_0_local << std::endl;
    MPI_Barrier(new_comm);

    MPI_Allreduce(&delta_0_local, &delta_0,1,MPI_DOUBLE,MPI_SUM,new_comm);
  ...
  //************************************************
   ...
   else
   {
    ...
    while(count < num_iter)
  	{
	   ...
	   // std::cout << "My rank is " << cart_rank  << " and my (d,z) is: " << scalar_Product_d_z << std::endl; 
	    ...
	    MPI_Allreduce(&scalar_Product_d_z, &global_scalar_Product_d_z,1,MPI_DOUBLE,MPI_SUM,new_comm);
    ...
    for(auto i=0;i<sizeproc; ++i)
     ...
     MPI_Allreduce(&scalar_Product_r_r, &delta_1,1,MPI_DOUBLE,MPI_SUM,new_comm);

      if(count == 1)
       std::cout << "I am " << cart_rank << " and my alpha : " << alpha << std::endl;
  ...
  //  std::cout<<"iteration := "<<count+1<<std::endl;
   // std::cout<<"I am rank := "<<cart_rank<<"and my value of isSolutionConverged is:="<<isSolutionConverged<<std::endl;
    ...
    if(count == 1)
       std::cout << "I am " << cart_rank << " and my beta : " << beta << std::endl;
  ...
  count++;
  std::cout << " Iteration " << count << " just ended\n";
  }   
